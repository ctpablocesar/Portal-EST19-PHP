<?php
    ...
    if (in_array($imagen_tipo, $imagen_tipo_permitido)) {
        if ($imagen_tamanio < 1024000) {
            $carpeta_destino = "../../Images/anuncios/";
            $ruta_destino = $carpeta_destino . $imagen_nombre;
            move_uploaded_file($imagen_temporal, $ruta_destino);
            $sql = "UPDATE anuncios SET titulo = '$titulo', descripcion = '$descripcion', fecha = '$fecha', link = '$link', imagen = '$imagen_nombre', status = '$status' WHERE id = '$id'";
            if (mysqli_query($conn, $sql)) {
                if ($imagen_actual != "") {
                    unlink("../../Images/anuncios/$imagen_actual");
                }
                echo "Anuncio actualizada correctamente";
            } else {
